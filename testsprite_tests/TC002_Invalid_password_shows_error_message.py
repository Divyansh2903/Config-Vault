...
async def run_test():
    ...
    try:
        ...
        # -> Type 'user_does_not_matter@example.com' into the email input (index 685), type 'wrong-password-123' into the password input (index 686), then click the 'Sign in' button (index 688). After submit, verify an 'invalid' error message is visible and that the URL still contains '/login'.
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div[2]/div[2]/div[3]/div/div[2]/form/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('user_does_not_matter@example.com')
